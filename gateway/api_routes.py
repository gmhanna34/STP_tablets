...
def register_api_routes(ctx):
    ...
    _MACRO_BUILDER_SUPPORTED_TYPES = {
        "ha_service", "wattbox_power", "moip_switch", "delay", "macro",
        "parallel", "condition",
    }

    def _macros_file_path() -> str:
        return getattr(ctx, "macros_path", _EDITABLE_FILES["macros"])

    def _normalize_yaml_keys_local(obj):
        bool_key_map = {True: "on", False: "off"}
        if isinstance(obj, dict):
            return {
                bool_key_map.get(k, k) if isinstance(k, bool) else k: _normalize_yaml_keys_local(v)
                for k, v in obj.items()
            }
        if isinstance(obj, list):
            return [_normalize_yaml_keys_local(i) for i in obj]
        return obj

    def _load_macros_document() -> tuple[dict, str]:
        path = _macros_file_path()
        with open(path, "r", encoding="utf-8") as f:
            raw = f.read()
        doc = yaml.safe_load(raw) or {}
        return _normalize_yaml_keys_local(doc), raw

    def _save_macros_document(doc: dict, actor: str, action: str) -> tuple[dict, int]:
        path = _macros_file_path()
        ...
        with open(path, "w", encoding="utf-8") as f:
            yaml.safe_dump(doc, f, default_flow_style=False,
                           sort_keys=False, allow_unicode=True)
        _refresh_macro_cache(doc)
        logger.info(f"macros.yaml updated by {actor}: {action}")
        db.log_action(actor, "macro_builder:save", "macros.yaml", action, "OK", 0)
        return {"success": True, "backup": os.path.basename(backup_path)}, 200

    def _collect_ha_entities_from_buttons(button_defs_local: dict) -> set:
        entities = set()
        ...
        def _collect(binding):
            ...
            if binding.get("entity"):
                entities.add(binding["entity"])
            for entity_id in (binding.get("entities") or []):
                entities.add(entity_id)

        for page_sections in button_defs_local.values():
            for section in page_sections:
                for item in section.get("items", []):
                    _collect(item.get("state"))
                    toggle = item.get("toggle")
                    if toggle:
                        _collect(toggle.get("state"))
                    _collect(item.get("badge"))
                    _collect(item.get("disabled_when"))
                _collect(section.get("disabled_when"))
        return entities

    def _refresh_macro_cache(macros_doc: Optional[dict] = None):
        if macros_doc is None:
            macros_doc, _ = _load_macros_document()
        new_macros = macros_doc.get("macros", {})
        new_buttons = macros_doc.get("buttons", {})
        ctx.macros_cfg = macros_doc
        if isinstance(getattr(ctx, "macro_defs", None), dict):
            ctx.macro_defs.clear()
            ctx.macro_defs.update(new_macros)
        else:
            ctx.macro_defs = new_macros
        if isinstance(getattr(ctx, "button_defs", None), dict):
            ctx.button_defs.clear()
            ctx.button_defs.update(new_buttons)
        else:
            ctx.button_defs = new_buttons
        ctx.ha_state_entities = _collect_ha_entities_from_buttons(ctx.button_defs)

    def _macro_step_supported(step_item: dict, depth: int = 0) -> bool:
        if depth > 6:
            return False
        step_type = step_item.get("type", "")
        if step_type not in _MACRO_BUILDER_SUPPORTED_TYPES:
            return False
        if step_type == "parallel":
            return all(_macro_step_supported(child, depth + 1)
                       for child in step_item.get("steps", []))
        if step_type == "condition":
            check = step_item.get("check", {})
            if check.get("type") not in ("ha_check", "wattbox_check"):
                return False
            return (
                all(_macro_step_supported(child, depth + 1)
                    for child in step_item.get("then", []))
                and all(_macro_step_supported(child, depth + 1)
                        for child in step_item.get("else", []))
            )
        return True

    def _macro_supported_for_builder(macro_def: dict) -> bool:
        return all(_macro_step_supported(step_item) for step_item in macro_def.get("steps", []))

    def _validate_builder_step(step_item: dict, path: str = "steps[0]") -> list[str]:
        errors = []
        step_type = step_item.get("type", "")
        if step_type not in _MACRO_BUILDER_SUPPORTED_TYPES:
            return [f"{path}: unsupported step type '{step_type}'"]

        if step_type == "ha_service":
            domain = str(step_item.get("domain", "")).strip()
            service = str(step_item.get("service", "")).strip()
            entity_id = str((step_item.get("data") or {}).get("entity_id", "")).strip()
            if not domain:
                errors.append(f"{path}: Home Assistant domain is required")
            if not service:
                errors.append(f"{path}: Home Assistant service is required")
            if not entity_id:
                errors.append(f"{path}: Home Assistant entity_id is required")
        elif step_type == "wattbox_power":
            if not str(step_item.get("device", "")).strip():
                errors.append(f"{path}: WattBox device is required")
            if step_item.get("action") not in ("on", "off", "cycle"):
                errors.append(f"{path}: WattBox action must be on/off/cycle")
        elif step_type == "moip_switch":
            if not str(step_item.get("tx", "")).strip():
                errors.append(f"{path}: MoIP transmitter is required")
            if not str(step_item.get("rx", "")).strip():
                errors.append(f"{path}: MoIP receiver is required")
        elif step_type == "delay":
            try:
                seconds = float(step_item.get("seconds", 0))
            except Exception:
                seconds = -1
            if seconds < 0:
                errors.append(f"{path}: delay seconds must be 0 or greater")
        elif step_type == "macro":
            if not str(step_item.get("macro", "")).strip():
                errors.append(f"{path}: nested macro key is required")
        elif step_type == "parallel":
            children = step_item.get("steps", [])
            if not children:
                errors.append(f"{path}: parallel group must contain at least one step")
            for idx, child in enumerate(children):
                errors.extend(_validate_builder_step(child, f"{path}.steps[{idx}]"))
        elif step_type == "condition":
            check = step_item.get("check", {}) or {}
            check_type = check.get("type", "")
            if check_type == "ha_check":
                if not str(check.get("entity", "")).strip():
                    errors.append(f"{path}: condition Home Assistant entity is required")
                if str(check.get("expect", "")).strip() == "":
                    errors.append(f"{path}: condition expected state is required")
            elif check_type == "wattbox_check":
                if not str(check.get("device", "")).strip():
                    errors.append(f"{path}: condition WattBox device is required")
                if str(check.get("expect", "")).strip() == "":
                    errors.append(f"{path}: condition expected state is required")
            else:
                errors.append(f"{path}: condition check type must be ha_check or wattbox_check")
            for idx, child in enumerate(step_item.get("then", [])):
                errors.extend(_validate_builder_step(child, f"{path}.then[{idx}]"))
            for idx, child in enumerate(step_item.get("else", [])):
                errors.extend(_validate_builder_step(child, f"{path}.else[{idx}]"))

        verify = step_item.get("verify")
        if verify not in (None, False, True):
            if not isinstance(verify, dict):
                errors.append(f"{path}: verify must be true/false or an object")
            else:
                try:
                    timeout = float(verify.get("timeout", 10))
                    retries = int(verify.get("retries", 2))
                    if timeout < 0:
                        errors.append(f"{path}: verify timeout must be 0 or greater")
                    if retries < 0:
                        errors.append(f"{path}: verify retries must be 0 or greater")
                except Exception:
                    errors.append(f"{path}: verify timeout/retries must be numeric")

        return errors

    def _sanitize_builder_step(step_item: dict) -> dict:
        step_type = step_item.get("type", "")
        clean = {"type": step_type}

        for attr in ("label", "message", "on_fail", "conditional"):
            value = step_item.get(attr)
            if value not in (None, ""):
                clean[attr] = value

        if step_type == "ha_service":
            clean["domain"] = str(step_item.get("domain", "")).strip()
            clean["service"] = str(step_item.get("service", "")).strip()
            data_block = step_item.get("data") if isinstance(step_item.get("data"), dict) else {}
            clean["data"] = {k: v for k, v in data_block.items() if v not in (None, "")}
        elif step_type == "wattbox_power":
            clean["device"] = str(step_item.get("device", "")).strip()
            clean["action"] = str(step_item.get("action", "on")).strip() or "on"
        elif step_type == "moip_switch":
            clean["tx"] = str(step_item.get("tx", "")).strip()
            clean["rx"] = str(step_item.get("rx", "")).strip()
        elif step_type == "delay":
            secs = step_item.get("seconds", 0)
            try:
                secs_num = float(secs)
                clean["seconds"] = int(secs_num) if secs_num.is_integer() else secs_num
            except Exception:
                clean["seconds"] = secs
        elif step_type == "macro":
            clean["macro"] = str(step_item.get("macro", "")).strip()
        elif step_type == "parallel":
            clean["steps"] = [_sanitize_builder_step(child) for child in step_item.get("steps", [])]
        elif step_type == "condition":
            check = step_item.get("check", {}) if isinstance(step_item.get("check"), dict) else {}
            clean["check"] = {k: v for k, v in check.items() if v not in (None, "")}
            clean["then"] = [_sanitize_builder_step(child) for child in step_item.get("then", [])]
            if step_item.get("else"):
                clean["else"] = [_sanitize_builder_step(child) for child in step_item.get("else", [])]
        ...
        if verify is True:
            ...
            verify_clean = {}
            for key in ("state", "timeout", "retries", "entity_id"):
                value = verify.get(key)
                if value not in (None, ""):
                    verify_clean[key] = value
            if verify_clean:
                clean["verify"] = verify_clean

        return clean

    def _macro_builder_payload(key: str, macro_def: dict) -> dict:
        builder_meta = macro_def.get("ui_builder", {}) if isinstance(macro_def.get("ui_builder"), dict) else {}
        return {
            "key": key,
            "label": macro_def.get("label", key),
            "description": macro_def.get("description", ""),
            "confirm": macro_def.get("confirm", ""),
            "category": builder_meta.get("category", ""),
            "managed": bool(builder_meta.get("managed")),
            "supported": _macro_supported_for_builder(macro_def),
            "steps": copy.deepcopy(macro_def.get("steps", [])),
            "raw": copy.deepcopy(macro_def),
        }
    ...
    @app.route("/api/macro-builder")
    def api_macro_builder_list():
        macro_items = []
        for key, macro_def in sorted(ctx.macro_defs.items()):
            builder_meta = macro_def.get("ui_builder", {}) if isinstance(macro_def.get("ui_builder"), dict) else {}
            macro_items.append({
                "key": key,
                "label": macro_def.get("label", key),
                "description": macro_def.get("description", ""),
                "confirm": macro_def.get("confirm", ""),
                "steps": len(macro_def.get("steps", [])),
                "supported": _macro_supported_for_builder(macro_def),
                "managed": bool(builder_meta.get("managed")),
                "category": builder_meta.get("category", ""),
            })

        moip_cfg = devices_data.get("moip", {}) if isinstance(devices_data, dict) else {}
        moip_transmitters = moip_cfg.get("transmitters", []) if isinstance(moip_cfg.get("transmitters", []), list) else []
        moip_receivers = moip_cfg.get("receivers", []) if isinstance(moip_cfg.get("receivers", []), list) else []

        wattbox_options = []
        try:
            wb_cfg = cfg.get("wattbox", {})
            devices = wb_cfg.get("devices", {})
            for device_key, dev in sorted(devices.items()):
                wattbox_options.append({
                    "key": device_key,
                    "label": dev.get("label", device_key),
                    "ip": dev.get("ip", ""),
                    "outlet": dev.get("outlet", ""),
                })
        except Exception:
            wattbox_options = []
        ...
        return jsonify({
            "macros": macro_items,
            "options": {
                "wattbox_devices": wattbox_options,
                "moip_transmitters": moip_transmitters,
                "moip_receivers": moip_receivers,
                "ha_domains": [
                    "switch", "light", "media_player", "climate", "button",
                    "input_boolean", "script", "lock", "cover",
                ],
                "ha_services": ["turn_on", "turn_off", "toggle", "press"],
            },
        }), 200

    @app.route("/api/macro-builder/<macro_key>")
    def api_macro_builder_get(macro_key: str):
        macro_def = ctx.macro_defs.get(macro_key)
        if not macro_def:
            return jsonify({"error": f"Unknown macro: {macro_key}"}), 404
        return jsonify(_macro_builder_payload(macro_key, macro_def)), 200

    @app.route("/api/macro-builder/preview", methods=["POST"])
    def api_macro_builder_preview():
        data = request.get_json(silent=True) or {}
        key = str(data.get("key", "")).strip() or "new_macro"
        ...
        for idx, step_item in enumerate(steps):
            if not isinstance(step_item, dict):
                ...
                continue
            errors.extend(_validate_builder_step(step_item, f"steps[{idx}]"))
        if errors:
        ...
        macro_entry = {
            ...
            "steps": [_sanitize_builder_step(step_item) for step_item in steps],
            "ui_builder": {
                "managed": True,
                "version": 1,
            },
        }
        confirm_msg = str(data.get("confirm", "")).strip()
        if confirm_msg:
            macro_entry["confirm"] = confirm_msg
        category = str(data.get("category", "")).strip()
        if category:
            macro_entry["ui_builder"]["category"] = category
        preview = yaml.safe_dump(
            {"macros": {key: macro_entry}},
    ...
    @app.route("/api/macro-builder", methods=["POST"])
    def api_macro_builder_save():
        ...
        if not all(c.isalnum() or c in ("_", "-") for c in key):
            return jsonify({"error": "key may only contain letters, numbers, underscores, and hyphens"}), 400
        ...
        try:
            macros_doc, _ = _load_macros_document()
            macros_section = macros_doc.setdefault("macros", {})
            existing = macros_section.get(key, {})
            builder_meta = existing.get("ui_builder", {}) if isinstance(existing, dict) and isinstance(existing.get("ui_builder"), dict) else {}
            category = str(data.get("category", "")).strip()
            builder_meta.update({
                "managed": True,
                "version": 1,
            ...
            if category:
                builder_meta["category"] = category
            elif "category" in builder_meta:
                builder_meta.pop("category", None)

            macro_entry = {
                "label": str(data.get("label", "")).strip() or key,
                "description": str(data.get("description", "")).strip(),
                "steps": [_sanitize_builder_step(step_item) for step_item in steps],
                "ui_builder": builder_meta,
            }
            confirm_msg = str(data.get("confirm", "")).strip()
            if confirm_msg:
                macro_entry["confirm"] = confirm_msg

            macros_section[key] = macro_entry
            result, status = _save_macros_document(
                macros_doc,
                get_tablet_id(),
                f"save:{key}",
            )
            result["macro"] = _macro_builder_payload(key, ctx.macro_defs.get(key, macro_entry))
            return jsonify(result), status
        except Exception as e:
            logger.error(f"Macro Builder save failed for {key}: {e}")
            return jsonify({"error": str(e)}), 500
    ...
    def api_macro_builder_delete(macro_key: str):
        try:
            macros_doc, _ = _load_macros_document()
            macros_section = macros_doc.get("macros", {})
            if macro_key not in macros_section:
                return jsonify({"error": f"Unknown macro: {macro_key}"}), 404
            del macros_section[macro_key]
            result, status = _save_macros_document(
                macros_doc,
                get_tablet_id(),
                f"delete:{macro_key}",
            )
            result["deleted"] = macro_key
            return jsonify(result), status
        except Exception as e:
            logger.error(f"Macro Builder delete failed for {macro_key}: {e}")
            return jsonify({"error": str(e)}), 500
